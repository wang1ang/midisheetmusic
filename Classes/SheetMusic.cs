...
namespace MidiSheetMusic
{
    ...
    public class SheetMusic : Control
    {
        ...
        // sheet status for editing: wang1ang
        private PointF prevClick;
         ...
         */
        private
        List<MusicSymbol> AddBars(List<ChordSymbol> chords, TimeSignature time, int lastStart)
        {
        ...
        {
            // wang1ang
            //Toub.Sound.Midi.MidiPlayer.Play(new Toub.Sound.Midi.NoteOn(0, Toub.Sound.Midi.GeneralMidiPercussion.BassDrum, 127));
            //Toub.Sound.Midi.MidiPlayer.Play(new Toub.Sound.Midi.NoteOn(300, 0, 60, 127));
            ...
            {
                staff = staffs[i];
                if (y < ypos && prevClick.Y < ypos)
                    break;
                g.TranslateTransform(0, ypos);
                if (ypos < prevClick.Y && prevClick.Y < ypos + staff.Height)
                {
                    staff.ShadeNotes(g, shadeBrush, pen, prevClick.X, false);
                }
                if (ypos < y && y < ypos + staff.Height)
                {
                    staff.ShadeNotes(g, shadeBrush, pen, x, true);
                }
                g.TranslateTransform(0, -ypos);
                ypos += staff.Height;
            }
            prevClick = new PointF(x, y);
        }
