...
namespace MidiSheetMusic
{
    ...
    public class Staff
    {
        ...
        public void Insert(int index, int notenumber)
        { // wang1ang
            // doto
        }
        public void ReDraw(Graphics g, Pen pen, int index, int xpos)
        { // wang1ang
            g.TranslateTransform(xpos, 0);
            symbols[index].Draw(g, pen, ytop);
            g.TranslateTransform(-xpos, 0);
            ReDrawLines(g, pen, symbols[index], xpos, null, 0);
        }
        ...
        public void Insert(Graphics g, SolidBrush shadeBrush, Pen pen, int index, int xpos)
        { // wang1ang
            /*
            ChordSymbol chord = new ChordSymbol();
            symbols.Insert(index, chord);

            g.TranslateTransform(xpos - 2, -2);
            g.FillRectangle(Brushes.White, 0, 0, chord.Width + 4, this.Height + 4);
            g.TranslateTransform(-(xpos - 2), 2);
            g.TranslateTransform(xpos, 0);
            chord.Draw(g, pen, ytop);
            g.TranslateTransform(-xpos, 0);
            */
            //int i = Seek(ref x);
            //Symbol
