...
namespace MidiSheetMusic
{
    ...
    public class Piano : Control
    {
        ...
        private void ShadeOneNote(Graphics g, Point pos, Brush brush)
        { // wang1ang
            int octave = pos.X / (WhiteKeyWidth * KeysPerOctave); // notenumber / 12;
            //octave -= 2;
            if (octave < 0 || octave >= MaxOctave)
                return;

            int delta = pos.X - octave * (WhiteKeyWidth * KeysPerOctave);
            int notescale = 0; // notenumber % 12;
            switch (delta / WhiteKeyWidth)
            {
                case 0: notescale = 0; break;
                case 1: notescale = 2; break;
                case 2: notescale = 4; break;
                case 3: notescale = 5; break;
                case 4: notescale = 7; break;
                case 5: notescale = 9; break;
                case 6: notescale = 11; break;
            }
            if (pos.Y <= BlackKeyHeight)
            {
                if (blackKeyOffsets[0] < delta && delta < blackKeyOffsets[1])
                    notescale = 1;
                else if (blackKeyOffsets[2] < delta && delta < blackKeyOffsets[3])
                    notescale = 3;
                else if (blackKeyOffsets[4] < delta && delta < blackKeyOffsets[5])
                    notescale = 6;
                else if (blackKeyOffsets[6] < delta && delta < blackKeyOffsets[7])
                    notescale = 8;
                else if (blackKeyOffsets[8] < delta && delta < blackKeyOffsets[9])
                    notescale = 10;
            }
            byte notenumber = (byte)(octave * 12 + notescale + 24);
            //ShadeOneNote(g, notenumber, brush);
            Toub.Sound.Midi.MidiPlayer.Play(new Toub.Sound.Midi.NoteOn(360, 0, notenumber, 127));
        }
        ...
        { // wang1ang
            ...
            ShadeOneNote(graphics, new Point(e.X - (margin + BlackBorder), e.Y - (margin + BlackBorder)), shade2Brush);
            graphics.TranslateTransform(-(margin + BlackBorder), -(margin + BlackBorder));
